...
| `backdrop_url` | `string \| null` | 가로형 배경 이미지 URL (히어로 배너 전용) |
| `category` | `string \| null` | CT_CL (TV드라마, 영화, TV 연예/오락, 키즈) |
| `score` | `float \| null` | 인기 점수 |

**동작:**
- 로그인 여부 무관하게 **popular top 5 고정** (이전의 hybrid 병합 제거)
- 배경 이미지는 `backdrop_url` 우선 사용, 없으면 `poster_url` fallback
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `top_vod` | `array \| null` | 상위 추천 VOD 목록 (히어로 캐러셀, **최대 5건**) — 이전의 단일 객체에서 배열로 변경 |
| `top_vod[].series_id` | `string` | full_asset_id |
| `top_vod[].asset_nm` | `string` | 자산명 |
| `top_vod[].poster_url` | `string \| null` | 포스터 URL |
| `top_vod[].backdrop_url` | `string \| null` | 가로형 배경 이미지 URL (히어로 배너 전용) |
...
- [x] `top_vod` 배열로 변경 — 캐러셀 렌더링 (4초 자동 전환, 인디케이터 포함)
- [x] `backdrop_url` 사용 — 배경 이미지 우선, 없으면 `poster_url` fallback
- [x] `source === "popular_fallback"` 분기 유지 (배너 태그 색·문구 변경)
...
```
source: "personalized"
top_vod: [ { series_id, asset_nm, poster_url, backdrop_url }, ... ]  ← 배열 최대 5건
  → 4초 자동 전환 캐러셀, backdrop_url 우선 배경
patterns: 7개
