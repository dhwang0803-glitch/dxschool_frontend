...
export default function HeroBanner({ vods }: { vods: VOD[] }) {
  ...
  return (
    <div className="relative w-full h-[480px] overflow-hidden">
      ...
      <Link href={`/series/${encodeURIComponent(vod.series_id)}`} className="absolute inset-0 flex items-end pb-16 px-10">
        <div>
          {vod.ct_cl && <span className="text-xs text-blue-400 font-medium tracking-widest uppercase">{vod.ct_cl}</span>}
          ...
          <div className="flex items-center gap-3 text-white/60 text-sm mb-3">
            ...
            {vod.rating && <><span>·</span><span>{vod.rating}</span></>}
            {vod.disp_rtm && <><span>·</span><span>{vod.disp_rtm}분</span></>}
          </div>
          {vod.smry && <p className="text-white/70 text-sm max-w-lg line-clamp-2">{vod.smry}</p>}
        </div>
      </Link>
