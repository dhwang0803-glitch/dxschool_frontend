...
| `feat/shopping-ad-popup` | `frontend/app/series/[series_id]/page.tsx`, `frontend/components/ShoppingAdPopup.tsx`, `frontend/lib/useAdSocket.ts`, `frontend/app/globals.css` |
...
| 광고 팝업 WebSocket 연동 | `WS /ad/popup` — playback_update → DB 조회 → ad_popup 전송 | 🔴 미구현 |
| 시청예약 알림 push | `WS /ad/popup` — reservation_alert 전송 | 🔴 미구현 |
...
## feat/shopping-ad-popup 브랜치 전용 규칙

### 담당 파일
- `frontend/lib/useAdSocket.ts` — WebSocket 연결 훅
- `frontend/components/ShoppingAdPopup.tsx` — 광고 팝업 컴포넌트
- `frontend/app/series/[series_id]/page.tsx` — 통합 (수정)
- `frontend/app/globals.css` — 애니메이션 추가 (수정)

### 작업 규칙
- WebSocket URL은 환경변수 `NEXT_PUBLIC_WS_URL`로 관리 (하드코딩 금지)
- 팝업 z-index: `z-[55]`, 토스트: `z-[60]` (GNB 검색/알림 `z-50`보다 위)
- 팝업 자동 최소화: 10초, 토스트 자동 제거: 3초
- playback_update 전송 간격: 0.5초

### 설계 문서
- [`docs/SHOPPING_AD_POPUP_SPEC.md`](docs/SHOPPING_AD_POPUP_SPEC.md)
